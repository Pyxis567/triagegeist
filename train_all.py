"""Run XGBoost, LightGBM, and MLP with 5-fold CV and save the best submission."""
import sys, os
...
from src.models import run_xgb, run_lgbm, run_nn
...
print(f"  >> CV Macro F1: {xgb_f1:.4f}")

# ── LightGBM ─────────────────────────────────────────────────────────────────
print("\n=== LightGBM (5-fold CV) ===")
lgbm_model, lgbm_f1 = run_lgbm(train_fe)
results["LightGBM"] = lgbm_f1
print(f"  >> CV Macro F1: {lgbm_f1:.4f}")

# ── Neural Network ────────────────────────────────────────────────────────────
print("\n=== Neural Network (MLP) ===")
nn_model, nn_scaler, nn_f1 = run_nn(train_fe, epochs=50, hidden_dims=(256, 128, 64),
...
if best_name == "XGBoost":
    ...
    with torch.no_grad():
        logits = nn_model(torch.tensor(X_te_s).to(_dev))
        preds = logits.argmax(dim=1).cpu().numpy() + 1

sub = pd.DataFrame({"patient_id": test_raw["patient_id"], "triage_acuity": preds})
out_path = f"submissions/{best_name.lower().replace(' ', '_')}_{results[best_name]:.4f}.csv"
